...
- **State**: `src/features/schulte/store/schulteStore.ts` (Zustand) is the single source of truth for both live game state (`grid`, `nextNumber`, `startTime`, `isActive`) and session history (`sessions`). It's persisted via `zustand/middleware`'s `persist` to `localStorage`, so session history and stats survive tab/browser restarts (cleared only if the user clears site data). Previously used `sessionStorage`, which cleared history on every tab close — changed after user feedback that history wasn't "saving".
